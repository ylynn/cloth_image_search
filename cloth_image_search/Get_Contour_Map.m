function [map] = Get_Contour_Map(img,TS)

[m n q] = size(img);

if m>n
    E0 = imresize(img,[TS,NaN]);
else
    E0 = imresize(img,[NaN,TS]);
end
[m n q] = size(E0);
...
Lab = colorspace('Lab<-RGB',E0); 
Lab_L = Lab(:,:,1);
Lab_a = Lab(:,:,2);
Lab_b = Lab(:,:,3);

[Ai] = color_components(Lab_L,Lab_a,Lab_b);
[Di] = parameter_from_color_components(Ai);
...
function [Z] =  color_components(A,B,C)

Z = zeros(size(A,1),size(A,2),6);
...
function [D] = parameter_from_color_components(A)
...
for i=1:num
    Ai = A(:,:,i);
    [Di] = Get_D_for_one_component(Ai);
    D(:,:,i) = Di;
end

function [D] = Get_D_for_one_component(I)
...
[Di] = Difference_for_matrix(double(E));

z = 3;
D = (1/z)*double(Di);
for i=2:z
    scale = 0.5;
    E = double(imresize(E,scale,'bilinear'));

    [Di] = Difference_for_matrix(E);
    D = D + (1/z)*double(imresize(Di,[mt nt],'bilinear'));

end

function [D] = Difference_for_matrix(I)

A = I;
B = I;
A(:,1:end-1) = I(:,2:end);
B(:,2:end) = I(:,1:end-1);
V = compare(abs(I-A),abs(I-B));

A = I;
B = I;
A(1:end-1,:) = I(2:end,:);
B(2:end,:) = I(1:end-1,:);
F = compare(abs(I-A),abs(I-B));
